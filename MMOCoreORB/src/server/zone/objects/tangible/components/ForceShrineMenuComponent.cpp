...
void ForceShrineMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player){
	...
	if (player->hasSkill("force_title_jedi_rank_02") && !creature->checkCooldownRecovery("isRank") && (!player->hasSkill("force_rank_dark_novice") || !player->hasSkill("force_rank_light_novice"))) {
		menuResponse->addRadialMenuItem(214, 3, "Apply for Dark Force Ranks");
		menuResponse->addRadialMenuItem(215, 3, "Apply for Light Force Ranks");
	}
}
...
int ForceShrineMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* creature, byte selectedID) {
	...
	//if (ghost->getAdminLevel() < 15)
		//return 0;
	if (selectedID == 213) {
		if (creature->getScreenPlayState("VillageJediProgression") && !creature->hasSkill("force_title_jedi_rank_02") && creature->getScreenPlayState("unlockedHolocron") >= 1){
	...
	if (selectedID == 214) {
		if (!creature->checkCooldownRecovery("isRank")){
			creature->sendSystemMessage("You cannot select a rank at this time. Check back in a few minutes.");
			return 0;
		}
		if (player->hasSkill("force_title_jedi_rank_02") && (!player->hasSkill("force_rank_dark_novice") || !player->hasSkill("force_rank_light_novice"))){
			creature->addCooldown("isRank", 60 * 5);
			ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
			box->setPromptTitle("Dark Force Ranks"); // Jedi Trials
			box->setPromptText("Congratulations on becoming a Dark Jedi. Relog for your robe.");
			ghost->addSuiBox(box);
			creature->sendMessage(box->generateMessage());
	...
	if (selectedID == 215) {
		...
		if (player->hasSkill("force_title_jedi_rank_02") && (!player->hasSkill("force_rank_dark_novice") || !player->hasSkill("force_rank_light_novice"))){
			creature->addCooldown("isRank", 60 * 5);
			ManagedReference<SuiMessageBox*> box = new SuiMessageBox(creature, SuiWindowType::NONE);
			box->setPromptTitle("Light Force Ranks"); // Jedi Trials
			box->setPromptText("Congratulations on becoming a Light Jedi. Relog for your robe.");
			ghost->addSuiBox(box);
			creature->sendMessage(box->generateMessage());
